...
> `ADMIN_PASSWORD` es la contraseña inicial de administración. Una vez que Marcela inicia sesión y cambia la contraseña desde la pestaña **🔐 Administración de acceso**, el sistema guarda solo el hash bcrypt en `data/admin_auth.json` y ya no usa `ADMIN_PASSWORD` en texto plano.

> **En Streamlit Cloud:** el filesystem es efímero, por lo que `admin_auth.json` se pierde al redesplegar. Para un cambio permanente en Cloud, actualiza `ADMIN_PASSWORD` en **Settings → Secrets**.
...
Las pestañas **⚙️ Configuración de búsquedas**, **📧 Reportes por correo** y **🔐 Administración de acceso** están protegidas con contraseña.

### Flujo de autenticación

| Situación | Cómo se autentica |
|---|---|
| Primera vez (sin `admin_auth.json`) | Usa `ADMIN_PASSWORD` de `.env` o Streamlit Secrets |
| Después del primer cambio de contraseña | Usa el hash bcrypt guardado en `data/admin_auth.json` |
| Streamlit Cloud tras redeploy | Vuelve a usar `ADMIN_PASSWORD` (filesystem efímero borra el hash) |

### Cómo Marcela cambia su contraseña

1. Entrar al dashboard y autenticarse con la contraseña actual
2. Ir a la pestaña **🔐 Administración de acceso**
3. Ingresar contraseña actual → nueva contraseña → confirmar
4. Clic en **Guardar nueva contraseña**

Requisitos de la nueva contraseña: mínimo 10 caracteres · mayúscula · minúscula · número · símbolo.

### Seguridad
- La contraseña **nunca se guarda en texto plano**
- Se almacena como hash bcrypt (12 rounds) en `data/admin_auth.json`
- `admin_auth.json` está en `.gitignore` — nunca se sube al repositorio
